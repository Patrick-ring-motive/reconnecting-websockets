...
class ReconnectingWebSockets {
    constructor(url, options = {}) {
        ...
        this._postTask = (callback, opts = {}) => {
            if (typeof scheduler !== 'undefined' && scheduler.postTask) {
                return scheduler.postTask(callback, { priority: "background", ...opts });
            }
    ...
    async waitNotBusy() {
        await this._sleep(1);
        try { await new Promise(r => this._postTask(r)); } catch {}
        await this._sleep(1);
        try { await new Promise(r => requestIdleCallback(r, { timeout: 2000 })); } catch {}
        await this._sleep(1);
    ...
    async start() {
        ...
        this.isStarted = true;
        let resolveSocket;
        this.waitingSocket = new Promise(resolve=>{resolveSocket = resolve;});
